...
from pychess.System.Log import log, timedLog
...
class BoardControl (Gtk.EventBox):    
    ...
    def button_press (self, widget, event):
        timedLog.write("buttonPress_"+str(event.button)+"_"+str(event.x)+"_"+str(event.y))
        return self.currentState.press(event.x, event.y, event.button)
    
    def button_release (self, widget, event):
        timedLog.write("buttonRelease_"+str(event.button)+"_"+str(event.x)+"_"+str(event.y))
